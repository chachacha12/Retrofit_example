...
interface HowlService {      //서버로 오고가는 api들을 관리해주는 인터페이스임  //서버와 앱 간의 연결역할..?
    ...
    @GET("api/cnts/{cnt_id}")
    fun getRequest(@Header("Authorization")authorization:String, @Path("cnt_id") cnt_id: String): Call<Baby>


    //이용자 모두조회
    //@Headers("Authorization:Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..6RVX6G_S-0AKm_ex-obiiSfgNOFTQq6RPfePJVAqjZE")
    @GET("api/cnts")
    fun getAllRequest(@Header("Authorization")authorization:String): Call<Babys>


    /*
     //logs 조회
     @GET("api/logs/{log_id}")
     fun getlogRequest(@Header("Authorization")authorization:String, @Path("log_id") log_id: String): Call<Baby>
      */
